...
census_api_key("96857c1a9ba1555b086460b064b4151f33a4f368", install = TRUE)
...
data_out_group <- acs_out4  %>% ungroup() %>% mutate(CountyToGroups = case_when(CountyTo %in% bay ~ "Bay Area",
...
#in
# acs_out <- acs_out %>% filter(MIGRATE1D >= 24)
# acs_out1IN <- acs_out %>% filter(is.na(MULTYEAR)) %>% mutate(PUMA_full = paste0(str_pad(STATEFIP,2,side = "left", pad =0)," ",str_pad(PUMA,5, side = "left", pad = 0)), MIG_full = paste0(str_pad(MIGPLAC1, 2, side = "left", pad = 0), " ", str_pad(MIGPUMA1,5, side = "left", pad = 0)))
# acs_out1a <- acs_out1 %>% inner_join((mig_puma_xwalk %>% select(MIG_fullx, PUMA_fullx )), by = c("PUMA_full" = "PUMA_fullx")) 
acs_out1bIN <- acs_out1a %>% group_by(MIG_full, MIG_fullx) %>% summarise( HH = sum(HHWT), PER = sum(PERWT))
...
acs_out3IN <- acs_out1bIN %>%  left_join( mig_puma_xwalkDa, by =c( "MIG_fullx" = "MIG_fullx")) %>% rename(County_T1 = cntyname) %>% 
  left_join( mig_puma_xwalkDa, by =c( "MIG_full" = "MIG_fullx")) %>% rename(County_T0 = cntyname) 
...
data_join <- inner_join(acs_out4, acs_out4IN, by = c("CountyTo" = "CountyFrom", "CountyFrom" = "CountyTo"))
...
data_net_group <- data_join  %>% ungroup() %>% mutate(CountyToGroups = case_when(CountyTo %in% bay ~ "Bay Area",
                                                                                CountyTo %in% sl ~ "LA/Orange",
                                                                                CountyTo %in% ie ~ "Inland Empire",
                                                                                TRUE ~ CountyTo))  %>% 
  mutate(CountyFromGroups = case_when(CountyFrom %in% bay ~ "Bay Area",
...
data_net_group1 <- data_net_group %>% ungroup() %>% group_by(CountyFromGroups, CountyToGroups ) %>% select(-CountyTo, -CountyFrom) %>% summarise_all(sum)
...
data_out_group_out %>% select(-Per_Out, -HH_Out, -HH_IN, -Per_IN, -Per_net) %>% spread(CountyFromGroups, HH_net) %>% arrange(desc(`Bay Area`)) %>% write.csv( "migration_net_HH.csv" )
data_out_group_out %>% select(-Per_Out, -HH_Out, -HH_IN, -Per_IN, -HH_net)  %>% spread(CountyFromGroups, Per_net) %>% arrange(desc(`Bay Area`)) %>% write.csv( "migration_net_Per.csv" )
data_out_group_out %>% write.csv("data_hhper_long.csv")
...
data_out_group_out %>% select(-Per_Out) %>% filter(abs(HH_Out) > 1000) %>% spread(CountyFromGroups, HH_Out) %>% arrange(desc(`Bay Area`)) %>% write.csv( "migration_out_HH.csv" ) %>% View()
